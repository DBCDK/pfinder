...
#perl -e 'for(sort@ARGV){($tag,$type)=split("=",$_,2);$name=$tag;$name=~s{(?<=[a-z])(?=[A-Z])}{_}g;$name=uc$name;print("public void ${tag}Open() throws XMLStreamException {writer.add(EVENT.${name}_OPEN);}public void ${tag}Close() throws XMLStreamException {writer.add(EVENT.${name}_CLOSE);}\n")}' \
